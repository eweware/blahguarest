...
//        }
    ...
    public InboxData getNextInbox(String groupId, String cohortId, Integer inboxNumber, Integer lastInboxNumber, Integer limit, Boolean safe) throws SystemErrorException, InvalidRequestException, ResourceNotFoundException {
        ...
        if (group != null) {
            ...
            Integer last;
            if (safe) {
                first = cohortInfo.getInt("FS", -1);
                last = cohortInfo.getInt("LS", -1);
            }
            else {
                first = cohortInfo.getInt("F", -1);
                last = cohortInfo.getInt("L", -1);
            }
            if (first == -1 || last == -1) {
                throw new SystemErrorException("No inbox is found for group id '" + groupId + "' cohort id '" + cohortId + "'", ErrorCodes.SERVER_SEVERE_ERROR);
            }
